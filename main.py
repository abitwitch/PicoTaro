...
def flip():
    return(envrandom.envRandomInt(0,1,1000))
...
def flipCard():
    flippedCard=[]
    for b in bytearray(reversed(epd.buffer_black[:len(epd.buffer_black)-len(moongraphic.image)])):
        b = (b & 0xF0) >> 4 | (b & 0x0F) << 4
        b = (b & 0xCC) >> 2 | (b & 0x33) << 2
        b = (b & 0xAA) >> 1 | (b & 0x55) << 1
        flippedCard.append(b)
    flippedCard=bytearray(flippedCard)
    epd.buffer_black=flippedCard+epd.buffer_black[len(epd.buffer_black)-len(moongraphic.image):]
